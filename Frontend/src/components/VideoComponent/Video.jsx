...
function Video() {
    ...
    const videoObj = UseReactQuery(`${import.meta.env.API_URL}/api/v1/videos/home-videos`, 'GET')
    const location = useLocation();
    const videoId = location.state;
    const videoInfo = UseReactQuery(`${import.meta.env.API_URL}/api/v1/videos/c/${videoId}`, 'GET')
    ...
    const onAddComment = useCallback(async (commentContent) => {
        try {
            const response = await fetch(`${import.meta.env.API_URL}/api/v1/comments/d/${videoId}`, {
                method: 'POST',
                headers: {
    ...
    const fetchVideoData = useCallback(async () => {
        try {
            const response = await fetch(`${import.meta.env.API_URL}/api/v1/videos/c/${videoId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
    ...
    useEffect(() => {
        (
            async () => {
                try {
                    if (userId) {
                        let response = await fetch(`${import.meta.env.API_URL}/api/v1/subscriptions/s/${userId}`, {
                            method: 'GET',
                            headers: {
                                'Authorization': `Bearer ${token}`
                            },
                        ...
                        response = await fetch(`${import.meta.env.API_URL}/api/v1/users/current-user`, {
                            method: 'GET',
                            headers: {
                                'Authorization': `Bearer ${token}`
                            },
    ...
    const handleSubscribeButton = async () => {
        ...
        let response = await fetch(`${import.meta.env.API_URL}/api/v1/subscriptions/c/${userId}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
            },
        ...
        if(response.ok) {
            let response2 = await fetch(`${import.meta.env.API_URL}/api/v1/subscriptions/s/${userId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
