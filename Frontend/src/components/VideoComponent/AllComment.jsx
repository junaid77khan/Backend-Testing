...
function AllComment({comment}) {
    ...
    const fetchLikeData = useCallback( async() => {
        const res = await fetch(`http://localhost:5000/api/v1/likes/boolComment/b/${comment?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
    ...
    const fetchDisLikeData = useCallback( async() => {
        const res = await fetch(`http://localhost:5000/api/v1/dislikes/dislikecommentbool/b/${comment?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
    ...
    const toggleLike = useCallback( async() => {
        const res = await fetch(`http://localhost:5000/api/v1/likes/toggle/c/${comment?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
    ...
    const toggleDisLike = useCallback( async() => {
        const res = await fetch(`http://localhost:5000/api/v1/dislikes/toggle/c/${comment?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
