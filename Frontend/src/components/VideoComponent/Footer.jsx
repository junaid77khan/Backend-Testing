...
function Footer({isOwner, videoData, subscribed, handleSubscribeButton}) {
    ...
    const fetchLikeData = useCallback( async() => {
        const res = await fetch(`https://video-sharing-app-backend-fcv2.onrender.com/api/v1/likes/boolLike/b/${videoData?._id}`,
        {
            method: 'GET',
            headers: {
    ...
    const fetchDislikeData = useCallback( async() => {
        const res = await fetch(`https://video-sharing-app-backend-fcv2.onrender.com/api/v1/dislikes/dislikebool/b/${videoData?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
    ...
    const toggleLike = useCallback( async() => {
        const res = await fetch(`https://video-sharing-app-backend-fcv2.onrender.com/api/v1/likes/toggle/v/${videoData?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
    ...
    const toggleDisLike = useCallback( async() => {
        const res = await fetch(`https://video-sharing-app-backend-fcv2.onrender.com/api/v1/dislikes/toggle/v/${videoData?._id}`,
        {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
